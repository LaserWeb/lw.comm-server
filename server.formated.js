                            ...
                            if (data.indexOf('ok') === 0) { // Got an OK so we are clear to send
                                ...
                                if (firmware == 'smoothie') {
                                    ...
                                    if (Array.isArray(wPos)) {
                                        ...
                                        if (send) {
                                            if (has4thAxis) {
                                                io.sockets.emit('wPos', {
                                                    x: parseFloat(xPos).toFixed(config.posDecimals),
                                                    y: parseFloat(yPos).toFixed(config.posDecimals),
                                                    z: parseFloat(zPos).toFixed(config.posDecimals),
                                                    a: parseFloat(aPos).toFixed(config.posDecimals)
                                                });
                                            } else {
                                                io.sockets.emit('wPos', {
                                                    x: parseFloat(xPos).toFixed(config.posDecimals),
                                    ...
                                    if (Array.isArray(mPos)) {
                                        ...
                                        if (send) {
                                            if (has4thAxis) {
                                                io.sockets.emit('wOffset', {
                                                    x: parseFloat(xOffset).toFixed(config.posDecimals),
                                                    y: parseFloat(yOffset).toFixed(config.posDecimals),
                                                    z: parseFloat(zOffset).toFixed(config.posDecimals),
                                                    a: parseFloat(aOffset).toFixed(config.posDecimals)
                                                });
                                            } else {
                                                io.sockets.emit('wOffset', {
                                                    x: parseFloat(xOffset).toFixed(config.posDecimals),
